...
export default function Cart() {
  ...
  const totalPrice = Object.values(cartItems).reduce((total, item) => {
    return (
      total +
      item.price *
        (1 - item.discountPercentage.toFixed(0) / 100).toFixed(2) *
        item.quantity
    );
  }, 0);
          ...
          <div className="w-full">
            ...
            {Object.entries(cartItems).map(([productId, item]) => {
              return (
                ...
                >
                  <button className="flex justify-start items-center gap-4 cursor-pointer" onClick={() => openProductDetails(item)}>
                    <img
                      src={item.thumbnail}
                      alt="item image"
                      className="w-20"
